...
def uno_proc_greedy(frame, cnm, t, old_comps, t_online, out, max_shifts_online):
    t_frame_start = time.time()
    if np.isnan(np.sum(frame)):
        raise Exception('Frame ' + str(t) + ' contains NaN')
    if t % 100 == 0:
        print(str(t) + ' frames have been processed in total. Total # of components is '
    ...
    Yr = frame_cor.reshape(-1, order='F')
    cnm.estimates.C_on[:t] = np.dot(cnm.estimates.Ab.T, Yr)
    if cnm.params.get('online', 'show_movie'):
        cnm.t = t
...
def demo():
    ...
    folder = data1
    cnm = cnm_init()
    cnm = base_prepare(folder, base_flag, cnm)
    E1, E2 = [0, 1], [2, 3]
    pc, T1 = baselineSimulation(E1, E2)
    print("Recommending T1: {}, with {}% correct.".format(pc, T1))
    T1 = -0.77
